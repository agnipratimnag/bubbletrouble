...
const int BUBBLE_DEFAULT_VY = 150;
const int BUBBLE_DEFAULT_AY = 1000;
const int PLAY_Y_HEIGHT1 = 450;
...
public:
    ...
    void nextStep(double t)
    {
        ...
        if ((vy > 0 && new_y > (PLAY_Y_HEIGHT1 - circle.getRadius()))
            ||
            ( vy < 0 && new_y < (0 + circle.getRadius())))
        {

            if (vy>0) vy=(-1)*BUBBLE_DEFAULT_VY;
            else vy=BUBBLE_DEFAULT_VY;
            ay = -ay;
            vy = vy+ay*t;
